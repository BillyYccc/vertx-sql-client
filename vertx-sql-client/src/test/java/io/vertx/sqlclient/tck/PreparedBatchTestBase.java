...
public abstract class PreparedBatchTestBase {
  ...
  public void testEmptyBatch(TestContext ctx) {
    connector.connect(ctx.asyncAssertSuccess(conn -> {
      ...
      conn.preparedBatch(statement("INSERT INTO immutable (id, message) VALUES (", ", ", ")"), batch, ctx.asyncAssertSuccess(result -> {
      }));
    }));
  }
  ...
  public void testInsert(TestContext ctx) {
    connector.connect(ctx.asyncAssertSuccess(conn -> {
      ...
      conn.preparedBatch(statement("INSERT INTO immutable (id, message) VALUES (", ", ", ")"), batch, ctx.asyncAssertSuccess(result -> {
        ...
        conn.preparedQuery(statement("SELECT * FROM immutable WHERE id=", ""), Tuple.of(79991), ctx.asyncAssertSuccess(ar1 -> {
          ...
          conn.preparedQuery(statement("SELECT * FROM immutable WHERE id=", ""), Tuple.of(79992), ctx.asyncAssertSuccess(ar2 -> {
            ...
            conn.preparedQuery(statement("SELECT * FROM immutable WHERE id=", ""), Tuple.of(79993), ctx.asyncAssertSuccess(ar3 -> {
              ...
              conn.preparedQuery(statement("SELECT * FROM immutable WHERE id=", ""), Tuple.of(79994), ctx.asyncAssertSuccess(ar4 -> {

...
class SimpleQueryCommandCodec<T> extends QueryCommandBaseCodec<T, SimpleQueryCommand<T>> {
  ...
  private void handleResultsetColumnCountPacketBody(ByteBuf payload) {
    ...
    if (encoder.socketConnection.isResultsetMetadataCacheEnabled()) {
      ...
      if (!metadataFollows) {
        MySQLRowDesc cachedRowDesc = encoder.socketConnection.textResultsetMetadataCache().get(cmd.sql());
        if (cachedRowDesc == null) {
          completionHandler.handle(CommandResponse.failure(new IllegalStateException(String.format("Fatal error: execute a query[%s] with resultset metadata cache option on but the client could not find the entry, make sure you have cached the resultset metadata before setting the variable resultset_metadata to NONE. The connection will be closed.", cmd.sql()))));

...
@Threads(8)
public class LargeSelectBenchmark extends MySQLBenchmarkBase {
  ...
  public void poolPreparedQuery(Blackhole blackhole) throws Exception {
    ...
    pool.preparedQuery("SELECT id, randomnumber from world")
      .execute(ar -> {
      if (ar.succeeded()) {
  ...
  public void pooledConnectionPreparedQuery(Blackhole blackhole) throws Exception {
    ...
    pool.getConnection(ar1 -> {
      if (ar1.succeeded()) {
        ...
        conn.preparedQuery("SELECT id, randomnumber from world")
          .execute(ar2 -> {
          conn.close();
          if (ar2.succeeded()) {
